...
void ProcessNewMessages()
{
	...
	while (enet_host_service(clientHost, &event, 0) > 0)
	{
		...
		case ENET_EVENT_TYPE_DISCONNECT:
			...
			printf("\nCliente: Has tenido una desconexion (Posible una intermitencia tuya o en el servidor)...  ");

			// to go the lobby browser
			octr.get()->CurrState = -1;
#ifdef PINE_DEBUG
			printf("statechange %d (-1) 13: enet Desconectado\n", octr.get()->stateChangeCounter++);
#endif
			break;
...
void PrintBanner(char show_name)
{
	...
	printf(" Cliente de OnlineCTR (presiona CTRL + C para salir)\n Build %s (%s)\n\n", __DATE__, __TIME__);
	printf(" Esta build NO TIENE soporte (por favor no molestes a los devs)\n\n");

	if (show_name == true) printf("Te has conectado a OnlineCTR: %s!\n", name);
}
...
void DisconSELECT()
{
	...
	if (((*hold.get()) & 0x2000) != 0)
	{
		...
		octr.get()->CurrState = -1;
#ifdef PINE_DEBUG
		printf("statechange %d (-1) 14: Presionaste SELECT\n", octr.get()->stateChangeCounter++);
#endif
		return;
	}
}
...
void StatePC_Launch_EnterPID()
{
	...
	StopAnimation();
	printf("Cliente: Esperando la conexion con el servidor...  ");
...
void StatePC_Launch_PickServer()
{
	...
	if (serverPeer != 0)
	{
		...
		printf("Algo ha sucedido con la transmision de datos (case 1), te hemos desconectado, lo sentimos\n");
		enet_peer_disconnect_now(serverPeer, 0);
		serverPeer = 0;
	}
	...
	switch (octr.get()->serverCountry)
	{
		...
		private_server_ip:
			...
			// IP address
			printf("\nIngresa una direccion IPV4: ");

			if (fgets(ip, sizeof(ip), stdin) == NULL)
			{
				printf("\nError: La IPV4 No es valida!\n");
		...
		private_server_port:
			// port number
			printf("Puerto del servidor (0-65535): ");
			...
			if (strlen(portStr) == 0)
			{
				printf("\nError: El puerto no puede estar vacio!\n");
	...
	StopAnimation();
	printf("Cliente: Intentando conectarse a \"");
	...
	if (clientHost == NULL)
	{
		fprintf(stderr, "Error: No se ha podido crear el host eNet!\n");
		exit_execv(3);
	}
	...
	if (serverPeer == NULL)
	{
		fprintf(stderr, "Error: No hay peers para iniciar la conexion eNet!\n");
		exit_execv(4);
	}
	...
	while (retryCount < MAX_RETRIES && !connected)
	{
		...
		{
			StopAnimation();
			printf("Cliente: Te has conectado exitosamente!  ");
		...
		{
			StopAnimation();
			printf("Error: La conexion ha fallado! reintentando %d/%d...  ", retryCount + 1, MAX_RETRIES);
			...
			if (retryCount >= MAX_RETRIES)
			{
				...
				octr.get()->CurrState = LAUNCH_PICK_SERVER;
#ifdef PINE_DEBUG
				printf("statechange %d LAUNCH_PICK_SERVER 16: La conexion fallo debido a que se reintento demasiadas veces\n", octr.get()->stateChangeCounter++);
#endif
...
#ifdef PINE_DEBUG
	printf("statechange %d LAUNCH_PICK_SERVER 17: Conexion al servidor fallida debido a una desconexion\n", octr.get()->stateChangeCounter++);
#endif
	octr.get()->boolClientBusy = 0;
	//unlike the above call to blockingWrite() in this function for octr, I don't think this is
	//necessary, but I'm doing it to be safe.
...
void StatePC_Lobby_HostTrackPick()
{
	...
	StopAnimation();
	printf("Cliente: Enviando pista al servidor...  ");
	...
	(octr.get())->CurrState = LOBBY_CHARACTER_PICK;
#ifdef PINE_DEBUG
	printf("statechange %d LOBBY_CHARACTER_PICK 18: Se ha seleccionado una pista\n", octr.get()->stateChangeCounter++);
#endif
}
...
#ifdef PINE_DEBUG
		printf("statechange %d LOBBY_WAIT_FOR_LOADING 19: Esperando a que el juego cargue\n", octr.get()->stateChangeCounter++);
#endif
	}
}
...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		printf("Por favor, introduce tu username:  ");
	...
	{
		fprintf(stderr, "Error: No se pudo inicializar el ENet!\n");

		return 1;
	}

	atexit(enet_deinitialize);
	printf("Cliente: Esperando a que se cargue el binario de OCTR...  ");
